...
export default function AdminSiswaPage() {
  ...
  const [filterJenjang, setFilterJenjang] = useState('all'); // Filter jenjang (X, XI, XII)
      ...
      (Array.isArray(filteredSiswa) ? filteredSiswa : []).forEach(s => {
        if (s.orangTuaSiswa && s.orangTuaSiswa.length > 0) {
          s.orangTuaSiswa.forEach(relation => {
            ...
            if (ortu && ortu.user) {
              // Password format: DDMMYYYY (from birth date)
              let passwordOrtu = s.nisn; // Fallback
              if (s.tanggalLahir) {
                try {
                  ...
                  console.error('Error formatting password:', e);
                }
              }
  ...
  const filteredKelasByJenjang = React.useMemo(() => {
    if (filterJenjang === 'all') return kelas;
    return kelas.filter(k => k.nama?.startsWith(filterJenjang));
  }, [kelas, filterJenjang]);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 border-2 border-emerald-200/60 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all cursor-pointer bg-white/50 hover:bg-white/70 disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-gray-100"
                  >
