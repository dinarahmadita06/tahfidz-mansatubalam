...
// POST - Reject tasmi registration with reason
export async function POST(request, { params }) {
  try {
    ...
    const { catatanPenolakan } = body;

    // Validation
    if (!catatanPenolakan || catatanPenolakan.trim() === '') {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { message: 'Tasmi berhasil ditolak', tasmi: updatedTasmi },
      { status: 200 }
    );
  } catch (error) {
    console.error('Error rejecting tasmi:', error);
    return NextResponse.json(
