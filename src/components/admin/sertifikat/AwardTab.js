    ...
    <div className="space-y-10">
        ...
        <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-emerald-100/60 p-6 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="space-y-2">
              <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Cari Nama/NISN</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white/50 border-2 border-emerald-50 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm font-bold transition-all"
                />
              </div>
            </div>

            <div className="space-y-2">
              <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Kelas</label>
              <select
                value={candidateFilters.kelasId}
                onChange={(e) => setCandidateFilters({ ...candidateFilters, kelasId: e.target.value })}
                className="w-full px-4 py-2.5 bg-white/50 border-2 border-emerald-50 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 text-sm font-bold cursor-pointer"
              >
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Tahun</label>
              <select
            ...
            <div className="flex items-end gap-2">
              <div className="flex-1 space-y-2">
                <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Gender</label>
                <select
            ...
            <div className="flex items-end justify-end">
              ...
              >
                <RefreshCw size={20} className={loadingCandidates ? 'animate-spin' : ''} />
              </button>
            </div>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-emerald-50/50 border-b border-emerald-100/40">
                  <th className="px-6 py-4 text-left text-xs font-bold text-emerald-800 uppercase tracking-widest">Siswa</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-emerald-800 uppercase tracking-widest">Kelas</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-emerald-800 uppercase tracking-widest">Juz</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-emerald-800 uppercase tracking-widest">Nilai</th>
                  <th className="px-6 py-4 text-right text-xs font-bold text-emerald-800 uppercase tracking-widest">Aksi Award</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-emerald-100/20">
                {loadingCandidates ? (
                  ...
                  <tr><td colSpan="5" className="px-6 py-12 text-center text-slate-400 font-bold italic">Tidak ada peserta yang ditemukan</td></tr>
                ) : (
                  candidates.map((tasmi) => (
                    <tr key={tasmi.id} className="hover:bg-emerald-50/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-slate-800 text-sm">{tasmi.siswa?.user?.name || '-'}</p>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{tasmi.siswa?.nisn || '-'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 text-center font-bold text-emerald-600 text-sm">Juz {tasmi.juzYangDitasmi}</td>
                      <td className="px-6 py-4 text-center">
                        <p className="text-sm font-black text-slate-700 leading-none">{tasmi.nilaiAkhir || '-'}</p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase mt-1">{tasmi.predikat || '-'}</p>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex justify-end items-center gap-2">
                          <div className="relative">
                            <select 
                              ...
                              className="pl-3 pr-8 py-2 bg-white border border-emerald-100 rounded-xl text-[10px] font-black uppercase tracking-wider outline-none focus:border-emerald-500 transition-all appearance-none cursor-pointer"
                            >
                              <option value="">Pilih Award...</option>
                              {categories.map(c => <option key={c.id} value={c.id}>{c.categoryName}</option>)}
                            </select>
                            <ChevronRight className="absolute right-2 top-1/2 -translate-y-1/2 text-emerald-500 rotate-90 pointer-events-none" size={12} />
                          </div>
                          <button
                            onClick={() => handleAddAward(tasmi)}
                            className="p-2 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 transition-all shadow-md active:scale-90"
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-black text-slate-800 tracking-tight">Daftar Terpilih Award Wisuda</h3>
        ...
        <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-amber-100/60 p-6 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Filter Kategori</label>
              <select
                value={selectedFilters.categoryId}
                onChange={(e) => setSelectedFilters({ ...selectedFilters, categoryId: e.target.value })}
                className="w-full px-4 py-2.5 bg-white/50 border-2 border-amber-50 rounded-xl outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 text-sm font-bold cursor-pointer"
              >
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Cari Peserta</label>
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="text"
                  ...
                  className="w-full pl-10 pr-4 py-2.5 bg-white/50 border-2 border-amber-50 rounded-xl outline-none focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 text-sm font-bold transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex items-end">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-amber-500 text-white font-black uppercase tracking-widest text-xs rounded-xl hover:bg-amber-600 transition-all shadow-md shadow-amber-100"
              >
        ...
        {/* Selected Table */}
        <div className="bg-white/70 backdrop-blur-md rounded-2xl border border-amber-100/60 overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="bg-amber-50/50 border-b border-amber-100/40">
                  <th className="px-6 py-4 text-left text-xs font-bold text-amber-800 uppercase tracking-widest">Siswa</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-amber-800 uppercase tracking-widest">Kelas</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-amber-800 uppercase tracking-widest">Kategori Award</th>
                  <th className="px-6 py-4 text-center text-xs font-bold text-amber-800 uppercase tracking-widest">Status Cert</th>
                  <th className="px-6 py-4 text-right text-xs font-bold text-amber-800 uppercase tracking-widest">Aksi</th>
                </tr>
              </thead>
                    ...
                    <tr key={rec.id} className="hover:bg-amber-50/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-slate-800 text-sm">{rec.student?.user?.name || '-'}</p>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-tighter">{rec.student?.nisn || '-'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="text-xs font-bold text-slate-600">{rec.student?.kelas?.nama || '-'}</span>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <div className="relative inline-block min-w-[180px]">
                          <select
                            ...
                            className="w-full pl-3 pr-8 py-1.5 bg-white border border-amber-200 rounded-lg text-xs font-bold text-amber-700 outline-none focus:border-amber-500 appearance-none cursor-pointer"
                          >
                            {categories.map(c => <option key={c.id} value={c.id}>{c.categoryName}</option>)}
                          </select>
                          <ChevronRight className="absolute right-2 top-1/2 -translate-y-1/2 text-amber-500 rotate-90 pointer-events-none" size={12} />
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        {rec.certificate ? (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-wider border border-blue-100 shadow-sm">
                            <FileCheck size={12} strokeWidth={3} /> Terbit
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-wider border border-slate-100">
                            Draft
                          </span>
                        )}
                      </td>
